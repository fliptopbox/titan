...
function parseTableData(table) {
    ...
    let rows = table.children[2].getElementsByTagName('tbody')

    // If the battle has not begun there are no nested tbody(s)
    if(!rows || !rows[0]) return data;
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        data.creatures.push({
            ...
            owner: data.id,
            element,
            damage: Number(damage.innerText)
        });
    }
